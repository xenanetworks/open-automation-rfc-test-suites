...
async def set_arp_request(
    ...
    for stream_info in stream_lists:
        ...
        peer_mac_address = await send_arp_request(port_struct, source_ip, destination_ip)
...
async def get_packet_header(port_struct: "Structure", source_ip:Union[IPv4Address, IPv6Address], destination_ip:Union[IPv4Address, IPv6Address]) -> str:
...
async def send_arp_request(
    ...
    await utils.apply(
        ...
        stream.packet.header.protocol.set([enums.ProtocolOption.ETHERNET, enums.ProtocolOption.IP]),
